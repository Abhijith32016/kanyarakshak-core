...
**Live demo:** [thunderous-puppy-34aa43.netlify.app](https://thunderous-puppy-34aa43.netlify.app)  
...
**Steady-state response time (warm backend):**
After excluding cold-start latency (a known limitation of free-tier hosting), the deployed chat endpoint — including a full round trip through the LLM inference call — averaged **~1.2 seconds** response time across repeated tests, with a cold-start penalty of ~30 seconds for the first request after the backend spins down from inactivity.

**Concurrent load test (Locust):**
Load testing with Locust confirmed the backend sustained **10 concurrent simulated users** across **107 requests** with a **0% failure rate**. Under concurrent load, median response time was ~3.1 seconds (p95: 12s, p99: 14s) for the LLM-backed chat endpoint — reflecting the expected latency tradeoff of free-tier shared compute rather than a systemic bottleneck, and demonstrating the system remains fully functional and error-free under simultaneous multi-user load.

| Metric | Value |
|---|---|
| Warm response time (chat endpoint) | ~1.2s average |
| Cold-start penalty (free-tier spin-down) | ~30s (first request only) |
| Concurrent users sustained (Locust) | 10 |
| Total requests tested | 107 |
| Failure rate under load | 0% |
| Median response time under load | ~3.1s |
| p95 response time under load | 12s |
| p99 response time under load | 14s |
...
1. User taps SOS button → frontend sends coordinates to `/api/v1/voice-distress`
2. Backend sets `ACTIVE` alert state in Redis, writes location to geospatial index
3. `GEOSEARCH` finds all users within 500m radius
4. Push notification dispatched via ntfy.sh to responder channel with live coordinates + resolution link
5. Background async worker polls user's live position every 2 minutes, re-broadcasting until resolved
6. Responder hits `/api/v1/resolve` → Redis state flips to `RESOLVED`, background loop terminates
